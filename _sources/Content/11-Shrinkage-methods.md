...
Our goal is to estimate the coefficients $\beta_1, \dots, \beta_p$. When the number of variables is large, one may expect several of the variables not to contribute significantly to predicting $Y$. For example, in the [cars dataset](sec-lab1-cars), some of the variables may not be good predictors for the price of the cars. Similarly, in the [gene expression example](E-gene-expression), most of the genes considered are probably unrelated to a given cancer. The difficulty in such problems is to identify which predictors are relevant. 

```{note}

In terms of getting the smallest *training error* possible, the model with the most variables will have training error no larger than that of any nested model with fewer variables.. However, in terms of *testing error* a model with a smaller number of variables will often perform better. 
```
...
Observe how each nonzero coefficient $\beta_i$ adds $\lambda$ to the objective function to minimize. When optimizing over $\beta \in \mathbb{R}^p$, there is thus a "competition" between minimizing the error $\|y-X\beta\|^2$ and the price $\lambda$ paid for including each variable. When $\lambda$ is large enough, each variable needs to significantly reduce the error in order to justify the cost of including it in the model. Otherwise, it is better to set $\beta_i = 0$. 
...
In theory, the above solves our problem of identifying relevant subsets of variables. However, unfortunately, the above optimization problem is a combinatorial optimization problem that can become computationally prohibitive as $p$ grows. We therefore need to look for alternatives. 
...
Notice how the term $\sum_{i=1}^p {\bf 1}_{\beta_i \ne 0}$ that previously counted the number of nonzero regression coefficients was replaced by a measure of "how large" the coefficients are (using the $\ell_2$ norm $\|\beta\|_2^2 = \sum_{i=1}^p \beta_i^2$). Thus, the above problem penalizes having large regression coefficients. 
...
A symmetric matrix $A \in \mathbb{R}^{n \times n}$ is said to be positive semidefinite if $x^T A x \geq 0$ for all $x \in \mathbb{R}^n$. Similarly, it is said to be positive definite if $x^T A x > 0$ for all $x \in \mathbb{R}^n \setminus \{\mathbf{0}\}$. Such matrices appear naturally in many areas of science and engineering. 
...
```
We obtain the following figure displaying the behavior of the coefficients as a function of the penalty parameter.
...
In contrast to Ridge regression, LASSO (Least Absolute Shrinkage and Selection Operator) regression does set coefficients to $0$ and selects relevant subsets of predictors in the linear regression problem. LASSO solves the following problem: 
...
Thus, LASSO regression minimizes the usual error, but looks for a solution in the $\ell_1$ ball of radius $t$. Observe that the shape of an $\ell_1$ ball is different from the shape of an $\ell_2$ ball. For example, let $v = (x,y) \in \mathbb{R}^2$. The standard $\ell_2$ unit ball is 
...
Let us now look at how the solution of the Ridge and the LASSO problems differ. Recall that each problem minimizes the error $\|y-X\beta\|_2^2$ of the linear regression, but over $\ell_2$ and $\ell_1$ balls respectively. We illustrate the difference in $\mathbb{R}^2$. 
